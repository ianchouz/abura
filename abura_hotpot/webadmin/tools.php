...
function setconfig($id,$xml){
    ...
    if($row['id']!=''){
        //更新
        $strSQL = "update ".$CFG->tbext."webconfig set xmlcontent=".pSQLStr($xml)." where id=".pSQLStr($id);
...
/**字串擷取**/
function htmlSubString($content,$maxlen=300,$append="..."){
            ...
            if (is_numeric($maxlen)){ //截取字數
                ...
                if ($wordend==false){
                    if ($maxLength>$maxlen){ //總字數大於要截取的字數，要在該行要截取
                        $outstr.=subString($value,0,$maxlen-$wordrows,$append);
                        $wordend=true;
                    }else{
                        $wordrows=$maxLength;
                        $outstr.=$value;
...
/***********按一定長度截取字串（包括中文）*********/
function subString($text, $start=0, $limit=12,$append="...") {
    return mysubstr2($text,$limit,$append);
}
...
class resize_img{
    ...
    function resize_image( $image_path ){
        $this->orgext = pathinfo($image_path, PATHINFO_EXTENSION);
...
/*return data*/
function build($success, $message = "") {
    $json = array(
        success => "$success",
        message => $message
    );
...
function buildFilehtml($upname = "filename", $limitedext = "", $size_mb = 10) {
    global $CFG;
    global $dao;
