        ...
        var reLoadList = function(gdir,pagesize,page,filterkey) {
          ...
          $.getJSON(o.listurl+"?gdir="+gdir +"&pagesize=" + pagesize +"&page=" + page +"&filterkey=" + filterkey +"&m="+Math.random(),"", function(json){
            ...
            $(".__blockimg").click(function() {
              ...
              //console.log('org:' + choice_array);
              if($(this).hasClass('__blockimg-selected')){

                var dataidx = parseInt($(this).attr("data-idx"));
                //console.log('do remove:' + dataidx);
                choice_array.remove(''+dataidx);
                $(this).removeClass('__blockimg-selected').attr('choice-idx','').find('.choice-info').remove();

                //console.log('remove ok:' + choice_array);                
                var _sel = 1;
                for(var i=0;i<choice_array.length;i++){                  
                  var idx = parseInt(choice_array[i]);
                  var now = $('.__blockimg').eq(idx);
                  now.attr('choice-idx',_sel-1).find('.choice-info').html(_sel);                    
                  _sel++;
                }

                $("#jBox_now_fileinfo").html("目前共選擇："+ $(".__blockimg-selected").size() +" 個");
                //console.log('remove-after:' + choice_array);
                return false;
              }
              var idx = parseInt($(this).attr("data-idx"));
              choice_array.push(''+idx);
              $(this).addClass("__blockimg-selected").attr('choice-idx',choice_array.length-1).append('<div class="choice-info">'+$(".__blockimg-selected").size()+'</div>');
              $("#jBox_now_fileinfo").html("目前共選擇："+ $(".__blockimg-selected").size() + " 個");
              //console.log('click-after:' + choice_array);
              //var idx = parseInt($(this).attr("data-idx"));
              //data.push(imagedatajson[idx]);
            });

            $("#btn_choiceIMG").click(function() {
              if(send) return false;
              send = true;
              data = [];
              //console.log('choice img:' + choice_array);
              var choice_size = choice_array.length;
              if (choice_size !=0){
                for(var i=0;i<choice_array.length;i++){
                    //var now = $('.__blockimg').eq(choice_array[i]);
                  var idx = parseInt(choice_array[i]);//parseInt(now.attr("data-idx"));
                  data.push(imagedatajson[idx]);
                }
                doCallback();
              }else{
              ...
              /*
              var choice_size = $(".__blockimg-selected").size();
              if (choice_size !=0){
                for(var i=0;i<choice_size;i++){
                  var idx = parseInt($(".__blockimg-selected").eq(i).attr("data-idx"));
                  data.push(imagedatajson[idx]);
                }
                doCallback();
              }else{
            ...
            $("#btn_clickdeleteimg").click(function(){
              if($(".__blockimg-selected").size() > 1){
                alert('請只選取一個進行刪除!!');
                return false;
              }
              if($(".__blockimg-selected").size() == 0){
                alert('您尚未選取檔案!!');
                return false;
              }              
              var idx = parseInt($(".__blockimg-selected").eq(0).attr("data-idx"));
