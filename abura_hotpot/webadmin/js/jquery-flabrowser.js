...
(function($) {
  ...
  jSFFLAworld = function(config) {
    var parent = $("body");
    if( config.width == undefined ) config.width = 600;
    ...
    if( config.showwidth == undefined ) config.showwidth = 200;
    ...
    var jBoxTree = $('<div id="jBox-dirtree" class="jBox-tree-menu" style="width:100px;float:left;height:' + height + 'px; overflow: auto"></div>');
    jBoxTree.appendTo(jBoxContent);
    var jBoxTabs = $('<div id="jBox-tabs" style="width:'+(width-103)+'px;float:left;height:' + height + 'px; overflow: auto;padding:0px;margin:0px;border:1px;"></div>');
    var jBoxTabsMenu = $('<ul><li><a href="#jBox-tabs-browser" style="font-size:10px;padding:2px 2px;">FLASH清單</a></li><li><a href="#jBox-tabs-upload" style="font-size:10px;padding:2px 2px;">檔案上傳</a></li></ul>');
    
    
    //
    jBoxTabsMenu.appendTo(jBoxTabs);

    var jBox_tabs_browser = $('<div id="jBox-tabs-browser"></div>');
    ...
    var jBox_tabs_browser_nowinfo = $('<div id="jBox_now_info" class="jBox_now_info"><div id="jBox_now_dir">目前目錄：</div><div id="jBox_now_fileinfo">目前選中檔案：</div></div><hr/>');
    jBox_tabs_browser_nowinfo.appendTo(jBox_tabs_browser);
    var jBox_tabs_browser_list = $('<div class="clearfix" id="photo-list" style="display: block;height:100%;"><ul></ul></div><div style="clear : both;"></div>');
    ...
    jBoxWraper.open = function() {
      ...
      jBoxWraper.cover();
      $('#jBox-dirtree').fileTree({listurl:fulladmin+'include/flash/flashBrowser.php',
       delurl:fulladmin+'include/flash/flashDelete.php',
       mkdirurl:fulladmin+'include/flash/flashDir.php',
 root: '', script: fulladmin+'include/jqueryFileTree/connectors/jqueryFileTree_FLA.php',showwidth:config.showwidth, showheight:config.showheight,fixwidth:config.fixwidth,fixheight:config.fixheight}, function(obj) {
        jBoxWraper.close();
        if (config.callback) {
      ...
      $("#jBox-tabs").tabs({ 
        select: function(e, ui) { 
          ...
          if (thistab.index==0){
            ...
            if ($("a[rel="+nn+"]").parent().hasClass("collapsed")){
              $("a[rel="+nn+"]").click();
            }
      ...
      $('.swfupload-control').each(function(){
        $(this).swfupload({
          upload_url: fulladmin+"include/flash/flashUpload.php",                     
      ...
      $("#btn_flupclose").click(function(){
        $("a[href=#jBox-tabs-browser]").click();
      });
      ...
      var listeners = {
        swfuploadLoaded: function(event){
          ...
          $('#fsUploadProgress', this).append('<li>元件載入成功，請點選[選擇檔案]按鈕，進行上傳，可同時點選多個。</li>');
        },
        ...
        uploadSuccess: function(event, file, serverData){           
          if (!serverData || serverData=="success"){
            $('#fsUploadProgress', this).append('<li>上傳成功 - '+file.name+'</li>');
          }else{
